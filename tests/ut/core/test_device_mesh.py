...
Note:
    All test methods decorated with ``@patch("hyper_parallel.core.dtensor.device_mesh._utils")``
    receive a ``mock_utils`` argument injected by the patch decorator.
"""
import copy
import os
import threading
...
def _patch_device_mesh_backend(mock_utils, world_size=8):
    """Patch ``device_mesh.dist`` and mirror its rank helpers onto ``mock_utils``.

    ``device_mesh`` reads rank information from ``torch.distributed`` while group
    creation stays in ``device_mesh._utils``; both are exposed through the same
    ``mock_utils`` handle that the ``@patch`` decorator injects. The patch is
    registered for teardown (``unittest`` ``addCleanup`` when available) so it
    never leaks into a sibling test.
    """
    del _ACTIVE_DIST_PATCHERS[:]
    patcher = patch("hyper_parallel.core.dtensor.device_mesh.dist")
    mock_dist = patcher.start()
    _ACTIVE_DIST_PATCHERS.append(patcher)
    mock_utils.get_rank = mock_dist.get_rank
    mock_utils.get_world_size = mock_dist.get_world_size
    mock_utils.get_process_group_ranks = mock_dist.get_process_group_ranks
...
def _patch_backend_for(fn):
    """Wrap a module-level test so its ``device_mesh.dist`` patch is torn down after it."""
    @functools.wraps(fn)
    def wrapper(*args, **kwargs):
        try:
            return fn(*args, **kwargs)
        finally:
            _stop_device_mesh_backend_patches()

    return wrapper


def _setup_device_mesh_mock(mock_utils, world_size=8):
    """Configure a hardware-free mock of the ``device_mesh`` module helpers."""
    _patch_device_mesh_backend(mock_utils, world_size)


def _stop_device_mesh_backend_patches():
    """Stop any ``device_mesh.dist`` patches started by the mock helpers."""
    while _ACTIVE_DIST_PATCHERS:
        _ACTIVE_DIST_PATCHERS.pop().stop()


@_patch_backend_for
@patch("hyper_parallel.core.dtensor.device_mesh._utils")
def test_device_mesh_view_cache_prefers_rooted_mesh_for_layout_rebuild(mock_utils):
    """
    ...
    """
    _setup_device_mesh_mock(mock_utils, world_size=8)
...
@_patch_backend_for
@patch("hyper_parallel.core.dtensor.device_mesh._utils")
def test_device_mesh_view_cache_keeps_root_mesh_when_view_matches_root_key(mock_utils):
    """
    ...
    """
    _setup_device_mesh_mock(mock_utils, world_size=4)
    EXISTING_COMM_GROUPS.clear()
    _DEVICE_MESH_MAP.clear()
...
class TestDeviceMesh(unittest.TestCase):
    ...
    def _setup_mock_platform(self, mock_utils, world_size=8):
        """Configure common mock attributes used across tests.

        Args:
            mock_utils: The MagicMock object injected by @patch.
            world_size: Value returned by mock_utils.get_world_size().
        """
        _patch_device_mesh_backend(mock_utils, world_size)
        self.addCleanup(_stop_device_mesh_backend_patches)

    def _make_2x4_mesh(self, mock_utils):
        """Set up mock and return a standard 2×4 (dp, tp) mesh via init_device_mesh."""
        self._setup_mock_platform(mock_utils, world_size=8)
        return init_device_mesh(device_type="npu", mesh_shape=(2, 4), mesh_dim_names=("dp", "tp"))

    def _make_2x2x2_mesh(self, mock_utils):
    ...
    def _make_2x2_mesh_no_backend(self, mock_utils, mesh_dim_names=("dp", "tp")):
    ...
    def _build_torchtitan_like_meshes_8card_cp2(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_init_device_mesh_basic(self, mock_utils):
        ...
        mock_utils.split_group.return_value = mock_group
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_init_device_mesh_caching(self, mock_utils):
        ...
        """
        self._setup_mock_platform(mock_utils, world_size=8)
        mock_utils.split_group.return_value = MagicMock()
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_with_tensor(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_with_list(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_with_numpy(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_with_none_mesh(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_without_dim_names(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_getitem_single_dim(self, mock_utils):
        """Test DeviceMesh __getitem__ with single dimension."""
        mesh = self._make_2x4_mesh(mock_utils)
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_getitem_multiple_dims(self, mock_utils):
        """Test DeviceMesh __getitem__ with multiple dimensions."""
        mesh = self._make_2x2x2_mesh(mock_utils)
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_getitem_no_dim_names_raises(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_getitem_invalid_dim_name(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_getitem_out_of_order(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_getitem_supports_contiguous_flatten_and_original_dim(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_getitem_rejects_non_contiguous_flatten_mix(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_concatenate_returns_unified_mesh_in_root_order(self, mock_utils):
        """Test concatenate combines explicit submeshes in root order."""
        mesh = self._make_2x2_mesh_no_backend(mock_utils)
        dp_mesh = mesh["dp"]
        tp_mesh = mesh["tp"]
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_concatenate_single_mesh_returns_input(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_concatenate_rejects_mismatched_root_meshes(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_concatenate_rejects_duplicate_mesh_dims(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_concatenate_rejects_meshes_out_of_root_order(self, mock_utils):
        """Test concatenate rejects meshes that do not follow root mesh order."""
        mesh = self._make_2x2_mesh_no_backend(mock_utils, mesh_dim_names=("tp", "dp"))
        dp_mesh = mesh["dp"]
        tp_mesh = mesh["tp"]
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_concatenate_supports_contiguous_flattened_dim(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_concatenate_preserves_root_mesh_after_layout_deepcopy(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_supports_torchtitan_build_mesh_primitives(self, mock_utils):
        """Test world->_unflatten->slice->_flatten matches TorchTitan's common mesh construction flow."""
        self._setup_mock_platform(mock_utils, world_size=16)
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_supports_torchtitan_8card_cp2_fsdp_semantics(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_supports_torchtitan_8card_cp2_concatenate_views(self, mock_utils):
        """Test concatenate rebuilds TorchTitan-like batch/cp and fsdp views on 8 cards."""
        _, dataloading_mesh, _, dense_mesh = self._build_torchtitan_like_meshes_8card_cp2(mock_utils)
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_supports_torchtitan_8card_cp2_axis_queries(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_unflatten_rejects_invalid_backend_override_keys(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_size_one_dims_skip_eager_group_creation(self, mock_utils):
        """Test singleton mesh dimensions do not eagerly create communication groups."""
        self._setup_mock_platform(mock_utils, world_size=1)
        mock_group = MagicMock()
        mock_utils.split_group.return_value = mock_group

        mesh = init_device_mesh("npu", (1, 1, 1), mesh_dim_names=("pp", "dp", "tp"))

        mock_utils.split_group.assert_not_called()
        ...
        mock_utils.split_group.assert_called_once_with(split_ranks=[[0]])

        group_again = mesh.get_group("pp")
        self.assertIs(group_again, mock_group)
        mock_utils.split_group.assert_called_once()

    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_fake_backend_dims_materialize_groups_lazily(self, mock_utils):
        ...
        world_mesh = init_device_mesh("npu", (8,), mesh_dim_names=("world",))
        mock_utils.split_group.reset_mock()
        ...
        self.assertEqual(mock_utils.split_group.call_count, 2)
        ...
        self.assertEqual(mock_utils.split_group.call_count, 3)
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_properties(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_repr(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_size(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_axis_methods(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_get_local_rank(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_get_rank_list_along_axis(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_get_global_shape(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_get_coordinate(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_root_mesh_property(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_sub_mesh_property(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_flatten(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_flatten_mapping(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_to_hash(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_assert_axis(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_get_all_groups(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_invalid_mesh_type(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_invalid_mesh_dim_names_length(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_invalid_mesh_dim_names_duplicate(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_get_sub_rank_list(self, mock_utils):
        ...
        """
        mock_utils.get_rank.return_value = 0
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_from_group_with_1d_group(self, mock_utils):
        ...
        self._setup_mock_platform(mock_utils, world_size=4)
        mock_group = MagicMock()
        mock_utils.get_process_group_ranks.return_value = [0, 1, 2, 3]
        mock_utils.get_created_group.return_value = False
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_from_group_with_nd_groups(self, mock_utils):
        ...
        mock_utils.get_process_group_ranks.side_effect = [
            [0, 1],  # dp group
            [0, 2],  # tp group
        ]
        mock_utils.get_created_group.return_value = False
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_from_group_invalid_mesh(self, mock_utils):
        ...
        self._setup_mock_platform(mock_utils, world_size=4)
        mock_group = MagicMock()
        mock_utils.get_process_group_ranks.return_value = [0, 1]
        mock_utils.get_created_group.return_value = False
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_get_devices_for_axis_with_str(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_get_devices_for_axis_with_int(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_get_devices_for_axis_invalid_dim(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_get_devices_for_axis_no_dim_names(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_get_devices_for_axis_invalid_rank(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_get_devices_for_axis_out_of_range(self, mock_utils):
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_init_backend_calls_init_process_group(self, mock_utils):
        ...
        call_order = []
        mock_utils.init_process_group.side_effect = lambda *a, **kw: call_order.append("init_process_group")
        mock_utils.get_world_size.side_effect = lambda *a, **kw: call_order.append("get_world_size") or 4

        DeviceMesh("npu", mesh=[[0, 1], [2, 3]], _init_backend=True)

        mock_utils.init_process_group.assert_called_once()
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_mesh_no_init_process_group_when_init_backend_false(self, mock_utils):
        ...
        mock_utils.init_process_group.assert_not_called()

    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_init_device_mesh_calls_init_process_group_when_init_backend_true(self, mock_utils):
        ...
        call_order = []
        mock_utils.init_process_group.side_effect = lambda *a, **kw: call_order.append("init_process_group")
        mock_utils.get_rank.side_effect = lambda *a, **kw: call_order.append("get_rank") or 0

        init_device_mesh("npu", (2, 2), mesh_dim_names=("dp", "tp"), init_backend=True)

        mock_utils.init_process_group.assert_called()
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_init_device_mesh_no_init_process_group_when_rank_list_provided(self, mock_utils):
        ...
        init_calls = []
        mock_utils.init_process_group.side_effect = lambda *a, **kw: init_calls.append(1)
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_init_device_mesh_get_rank_failure_raises_runtime_error(self, mock_utils):
        ...
        """
        self._setup_mock_platform(mock_utils, world_size=4)
        mock_utils.get_rank.side_effect = RuntimeError("process group not initialized")
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_type_valid_torch_platform(self, mock_utils):
        """Test that all valid device_type values are accepted.

        Scenario: Create DeviceMesh with each of "cpu", "cuda", "npu".
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_device_type_invalid_raises(self, mock_utils):
        """Test that invalid device_type values raise ValueError.
        ...
        invalid value and the valid device types.
        """
        self._setup_mock_platform(mock_utils, world_size=4)
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_get_current_mesh_raises_when_no_context(self, mock_utils):
        """
        ...
        """
        self._setup_mock_platform(mock_utils, world_size=2)
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_with_mesh_sets_get_current_mesh(self, mock_utils):
        """
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_with_mesh_as_returns_self(self, mock_utils):
        """
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_nested_with_inner_mesh_is_current(self, mock_utils):
        """
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_with_mesh_pops_stack_on_exception(self, mock_utils):
        """
    ...
    @patch("hyper_parallel.core.dtensor.device_mesh._utils")
    def test_get_current_mesh_thread_local_isolation(self, mock_utils):
        """
