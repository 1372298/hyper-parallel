...
def _empty_cache() -> None:
    """Collect host and device memory around checkpoint I/O."""
    gc.collect()
...
def _materialize_hyper_optimizer_state(
        optimizer: Any,
        params: List[torch.Tensor],
) -> bool:
    """Create lazy Hyper AdamW/Muon state without executing an update step."""
    if isinstance(optimizer, HyperMuon):
        state_keys = ("momentum_buffer",)
    elif isinstance(optimizer, HyperAdamW):
    ...
    with SkipDTensorDispatch(no_skip={torch.zeros_like}):
        for group in optimizer.param_groups:
            ...
            for param in group["params"]:
                ...
                if isinstance(optimizer, HyperAdamW) and group.get("amsgrad", False):
                    state.setdefault(
                        "max_exp_avg_sq",
                        torch.zeros_like(param, memory_format=torch.preserve_format),
                    )
...
def initialize_optimizer_state(optimizer: Any) -> bool:
    ...
    Hyper AdamW and Muon states are created directly. This avoids running Muon's
    HSDP communication and Newton-Schulz update merely to allocate zero momentum.
    Unknown optimizer implementations retain the parameter-neutral priming-step
    fallback, with ``lr`` and ``weight_decay`` forced to zero.
    ...
    for leaf_optimizer, params in leaf_parameters.items():
        if _materialize_hyper_optimizer_state(leaf_optimizer, params):
            continue
        saved_hyperparams = [
