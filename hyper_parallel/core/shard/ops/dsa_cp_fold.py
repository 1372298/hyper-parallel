...
def _index_tensor(values: Sequence[int], ref):
    ...
    The cache key carries the backend of ``ref``, and a hit is re-validated against it:
    a single process can hold both backends (torch and MindSpore UTs share one pytest
    run, and the platform singleton is rebound between them), and handing a torch index
    to MindSpore's ``index_select`` raises a bare TypeError from the converter.
    """
    backend = type(ref).__module__.split(".")[0]
    key = (backend, tuple(values))
...
class _UnfoldPrefix(platform.Function):
    ...
    def forward(ctx, x, seq_shard_id, seq_shards, block_id, seq_dim):  # pylint: disable=arguments-differ
        """One folded block's causal key prefix."""
    ...
    def backward(ctx, grad_output):  # pylint: disable=arguments-differ
        """Scatter the prefix-shaped gradient back onto the folded layout.

        The autograd default for ``index_select`` is a full-length ``index_add``, and
...
class _UnfoldPrefixPair(platform.Function):
    ...
    def forward(ctx, x, seq_shard_id, seq_shards, seq_dim):  # pylint: disable=arguments-differ
        """Both folded blocks' causal key prefixes, gathered in one pass."""
