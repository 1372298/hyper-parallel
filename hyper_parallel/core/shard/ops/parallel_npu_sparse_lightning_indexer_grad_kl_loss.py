...
# ============================================================================
"""Distributed implementation for npu_sparse_lightning_indexer_grad_kl_loss operator."""
import copy
...
def _normalize_sparse_grad_kl_loss_args(
        query,
        key,
        query_index,
        key_index,
    ...
    Args:
        query: Main attention query (q_t).
        key: Main attention key (K_t).
        query_index: Lightning Indexer query input (q̃_t).
        key_index: Lightning Indexer key input (K̃_t).
        weights: Lightning Indexer weight coefficient (W_t).
        sparse_indices: Sorted token indices for key/key_index.
        softmax_max: Attention softmax max values.
        softmax_sum: Attention softmax sum values.
        scale_value: Scaling factor (float scalar).
...
class NpuSparseLightningIndexerGradKlLossDistributedOp(DistributedOp):
    """Distributed operator for npu_sparse_lightning_indexer_grad_kl_loss.
    ...
    Platform differences in ``preprocess``:
      - MindSpore: all 17 arguments are positional (no kwargs).
      - PyTorch: the 9 mandatory positional args are positional; optional
    ...
    def preprocess(self, args: tuple, kwargs: dict) -> tuple:
        ...
        """
        norm_args, _ = _normalize_sparse_grad_kl_loss_args(*args, **kwargs)
        query_index = norm_args[2]
        key_index = norm_args[3]
        weights = norm_args[4]
        scale_value = norm_args[8]
        layout_str = norm_args[13]
    ...
    def _validate_input_layouts(
        ...
        """
        op = "npu_sparse_lightning_indexer_grad_kl_loss"
    ...
    def get_expand_impl(  # pylint: disable=W0237
            ...
            infer_result: tuple,
            cache_values: list,
            extra_args: Optional[tuple] = None,  # pylint: disable=unused-argument
    ) -> Optional[Callable]:
        ...
        if layout_str == 'BSND':
            ...
            def _bsnd_cp_impl(*args, **kwargs):
                ...
                if len(args) > 9:  # MindSpore: all 17 positional args
                    local_k_rope = args[10]
                    sliced_k_rope = (
                    ...
                    outs = func(
                        ...
                        *args[4:10], sliced_k_rope, *args[11:], **kwargs
                    )
                else:
                    # PyTorch: 9 positional, optional tensors in kwargs
                    new_args = list(args)
                    new_args[1] = sliced_k
        ...
        def _tnd_impl(*args, **kwargs):
            ...
            if len(args) > 9:  # MindSpore
                return func(*args[:11], adj_q, adj_k, *args[13:], **kwargs)
