...
py::array_t<T> build_sample_idx(const py::array_t<int32_t> &sequence_lengths,
                                ...
                                int64_t num_tokens_per_epoch, bool drop_last_partial_sequence,
                                bool add_extra_token_to_sequence) {
  ...
  const int64_t num_samples = drop_last_partial_sequence ? available_tokens / sequence_length
                                                         : (available_tokens + sequence_length - 1) / sequence_length;
  py::array_t<T> sample_index({static_cast<py::ssize_t>(num_samples + 1), static_cast<py::ssize_t>(2)});
...
void build_blending_indices(py::array_t<int16_t, 0> &dataset_index, py::array_t<int64_t, 0> &dataset_sample_index,
  ...
  constexpr int64_t kMaxDatasets = static_cast<int64_t>(std::numeric_limits<int16_t>::max()) + 1;
