...
In `platform/torch/` and `platform/mindspore/`, framework imports belong **inside methods** with `# pylint: disable=C0415`. In **non-platform** code, prefer module-level imports; local imports there are only for documented exceptions (see `.agent/rules/code-style.md`).
